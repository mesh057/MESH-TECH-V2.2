...
class BotInstance {
    constructor(number, authDir) {
        ...
        this.sessionBackupSent = fs.existsSync(path.join(this.authDir, '.session_backup_sent'));
        this.sessionBackupInFlight = false;
        this.sessionBackupTimer = null;
    }
    ...
    async adoptPairingSession(sourceDir) {
        ...
        this.connecting = false;
        this.openHandled = false;
        this.sessionBackupSent = false;
        this.sessionBackupInFlight = false;
        if (this.sessionBackupTimer) clearTimeout(this.sessionBackupTimer);
        this.sessionBackupTimer = null;
        try {
            if (this.sock) {
    ...
    async connect() {
        ...
        try {
            ...
            registerMessageHandler(this.sock, this.commands, {
                ...
                ownerNumber: this.number,
                instanceNumber: this.number,
    ...
    registerEvents() {
        ...
        this.sock.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'open') {
                if (this.openHandled) {
                    logger.debug(`[BotInstance:${this.number}] Duplicate open event ignored.`);
                    return;
                }
                ...
                this.presenceManager.markReady();
                ...
                if (selfJid) {
                    ...
                    // Send the session backup at most once for this auth directory.
                    // Reconnect/open events must never create a message flood.
                    if (!this.sessionBackupSent && !this.sessionBackupInFlight) {
                        this.sessionBackupInFlight = true;
                        this.sessionBackupTimer = setTimeout(async () => {
                            try {
                                const credsPath = path.join(this.authDir, "creds.json");
                                if (!fs.existsSync(credsPath)) {
                                    throw new Error('creds.json not persisted yet');
                                }

                                const initialMsg = await this.sock.sendMessage(selfJid, {
                                ...
                                const notice = `╭━━━〔 *MESH-TECH CLOUD SESSION* 〕━━━┈⊷\n` +
                                    ...
                                    `┃ 🔑 *Your SESSION_ID:*\n` +
                                    `╰━━━━━━━━━━━━━━━━━━━━━━┈⊷`;
                                await this.sock.sendMessage(selfJid, { text: notice });
                                await this.sock.sendMessage(selfJid, { text: sessionId });
                                fs.writeFileSync(path.join(this.authDir, '.session_backup_sent'), String(Date.now()));
                                this.sessionBackupSent = true;
                                logger.info(`[BotInstance:${this.number}] Session backup sent once.`);
                            } catch (e) {
                                logger.error(`[BotInstance:${this.number}] Session backup delivery failed: ${e.message}`);
                            } finally {
                                this.sessionBackupInFlight = false;
                                this.sessionBackupTimer = null;
                            }
    ...
    handleSessionError(error) {
        ...
        // A Bad MAC, bad-session status, or signal-session failure means the
        // WhatsApp auth state is corrupted. Quarantine only auth files so the
        // tenant's settings/group data remain intact for fresh pairing.
        const fatal = isFatalSignalSessionError(error);
        if (fatal) {
            ...
            try {
                ...
                logger.warn(`[BotInstance:${this.number}] Failed to quarantine corrupted auth files: ${cleanupError.message}`);
            }
            logger.error(`[BotInstance:${this.number}] Critical session corruption. Fresh pairing required.`);
        } else if (this.reconnectAttempts > 5) {
            this.sessionResetRequired = true;
            logger.error(`[BotInstance:${this.number}] Critical session failure. Manual restart or fresh pairing required.`);
        }
