...
class BotInstance {
    ...
    handleSessionError(error) {
        ...
        // DO NOT delete authDir here. Baileys often recovers from "Bad MAC" errors
        // after a simple restart. Deleting the folder forces the user to re-pair
        // every time there is a transient network or protocol error.
        logger.error(`[BotInstance:${this.number}] Corrupted WhatsApp signal session detected (${error?.message || 'Bad MAC'}). A manual restart or fresh pairing may be required if it fails to reconnect.`);
    }
