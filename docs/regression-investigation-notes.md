...
`lib/BotInstance.js` now guarantees command loading during every connection and restores the useful post-pairing welcome guidance. The per-instance `PresenceManager` now refreshes always-online presence, emits configurable typing or recording presence for incoming chats, and isolates settings per BotInstance. Status messages are auto-viewed and optionally reacted to when enabled. `package.json` exposes named regression scripts: `test:events`, `test:pairing`, `test:instances`, `test:multitenant`, `test:commands`, `test:status`, and the aggregate `test:regression`.
